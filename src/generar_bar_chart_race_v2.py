...
def main():
    ...
    # 1. DESCARGA DE DATOS (Tabla corregida a 'users')
    ...
    # 2. MATEMÁTICA ACUMULATIVA (Sin frames intermedios confusos)
    ...
    for u in jugadores:
        ...
        for idx, match in enumerate(partidos_jugados, start=1):
            ...
            # Extraemos los goles reales guardados para mostrarlos en el título de la animación
            goles_h = int(match["home_score"])
            goles_a = int(match["away_score"])
            
            # Formateamos el título del juego con su resultado oficial
            ...
            historial_puntos.append({
                "Jugador": u["name"],
                # Este campo es el que Plotly usa para pintar el texto animado arriba
                "Frame_Animacion": label_frame_dinamico, 
                "Puntos": puntos_acumulados,
                "Puntos_Label": puntos_acumulados
            })

    df_animar = pd.DataFrame(historial_puntos)

    # 🎨 3. RENDERIZADO CON PALETA PREMIUM
...
# ⚙️ 4. CONTROL TOTAL DE ANIMACIÓN, RITMO Y DISEÑO LIMPIO (FÍSICA CORREGIDA)
    # 🟢 AJUSTE CLAVE: Bajamos la duración del deslizamiento (de 1500ms a 700ms) 
    # para que el cambio de ranking sea rápido y reactivo, eliminando el delay confuso.
    duracion_cuadro = 2200  # Tiempo total que se queda quieto el juego en pantalla
    duracion_deslizar = 700 # Tiempo que tarda la barra en moverse a su nuevo puesto
    ...
    fig.update_layout(
        ...
        # Margen inferior aumentado (b=120) para dar espacio a los botones caídos
        ...
        # 🟢 BOTONES MÁS ABAJO: Pasamos de -0.15 a -0.22 para liberar el eje X por completo
        updatemenus=[{
            "type": "buttons",
            "direction": "left",
            "pad": {"r": 10, "t": 10},
            "showactive": False,
            "x": -0.05, 
            "y": -0.22, # 🔥 Empujado hacia abajo de forma segura
            "xanchor": "left",
            "yanchor": "top",
            "buttons": [{
                "label": "▶️ Play",
    ...
    # 🟢 5. INYECCIÓN PURA DE TEXTO EN CADA FRAME
