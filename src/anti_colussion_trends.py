...
class QuinielaTrendCollusionEngine:
    def __init__(self, raw_predictions: list, raw_users: list, raw_matches: list):
        """
        Engine de Auditoría Sutil.
        Detecta alianzas evaluando la tendencia direccional (1X2) en lugar del marcador exacto.
        """
        audit_params = config.get_section("anti_collusion_audit")
        # Podemos ser un poco más estrictos con el umbral aquí, ya que coincidir en 1X2 es más fácil que en goles
        self.max_consensus = audit_params.get("max_expert_consensus", 0.85)
        self.alert_threshold = audit_params.get("similarity_alert_threshold", 0.85) # Umbral sugerido más alto
        ...
        # 🟢 Construcción de la matriz direccional 1X2
        self.df_matrix = self._construir_matriz_tendencias(raw_predictions)
        
    def _derivar_tendencia(self, home_score, away_score):
        """Convierte goles crudos en mercado 1X2."""
        try:
            h = int(home_score)
            a = int(away_score)
            if h > a: return "L"
    ...
    def _construir_matriz_tendencias(self, predictions: list) -> pd.DataFrame:
        if not predictions: return pd.DataFrame()
        df = pd.DataFrame(predictions)
        
        print("[DEBUG] Derivando tendencias 1X2 (Local/Empate/Visitante)...")
        
        # 🟢 Aplicamos la regla lógica para extraer la tendencia oculta
        df["tendencia_1x2"] = df.apply(lambda row: self._derivar_tendencia(row["home_score"], row["away_score"]), axis=1)
        
        df_pivot = df.pivot(index="user_id", columns="match_id", values="tendencia_1x2")
            ...
            for j in range(i + 1, len(usuarios)):
                ...
                if similitud_critica >= self.alert_threshold:
                    ...
                    alertas.append({
                        "Jugador_A": user_a,
                        "Jugador_B": user_b,
                        "Similitud_Tendencia_Crítica": f"{similitud_critica*100:.1f}%",
                        "Partidos_Misma_Tendencia_Totales": f"{coincidencias_exactas_totales} de {total_jugados_mutuamente}",
                        "Mismo_Clan_Denso": mismo_clan
                    })
                    
        df_reporte = pd.DataFrame(alertas)
        return df_reporte.sort_values(by="Similitud_Tendencia_Crítica", ascending=False) if not df_reporte.empty else pd.DataFrame()

    def generar_reportes_visuales(self, df_similarity: pd.DataFrame, linkage_tree: np.ndarray, G: nx.Graph, output_dir: str = "data/reports_trends"):
        os.makedirs(output_dir, exist_ok=True)
        usuarios = df_similarity.index.tolist()

        fig_heat = px.imshow(df_similarity, text_auto=".2f", color_continuous_scale="Purples", 
                             title="Matriz Térmica de Tendencias (1X2 - Partidos Críticos)")
        fig_heat.write_html(os.path.join(output_dir, "heatmap_tendencias.html"))

        fig_dendro = ff.create_dendrogram(1.0 - df_similarity.values, orientation='left', labels=usuarios)
        fig_dendro.update_layout(width=800, height=600, title="Dendrograma de Alianzas Direccionales (1X2)")
        fig_dendro.write_html(os.path.join(output_dir, "dendrograma_tendencias.html"))
        ...
        fig_net = go.Figure(data=[
            ...
            go.Scatter(x=node_x, y=node_y, mode='markers+text', text=list(G.nodes()), textposition="bottom center",
                       marker=dict(showscale=True, size=20, color="purple", line_width=2))
        ])
        fig_net.update_layout(title="Red de Alianzas Sutiles (Misma Intuición Direccional)", showlegend=False, 
                              xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
                              yaxis=dict(showgrid=False, zeroline=False, showticklabels=False))
        fig_net.write_html(os.path.join(output_dir, "grafo_red_tendencias.html"))
        print(f"[VISUALIZACIÓN] Gráficos de tendencias interactivos guardados en: {os.path.abspath(output_dir)}")

# =====================================================================
# EJECUCIÓN
# =====================================================================
if __name__ == "__main__":
    print("🔌 Conectando a Supabase para Auditoría Sutil (Tendencias 1X2)...")
    try:
        ...
        if pronosticos and jugadores and matches:
            engine = QuinielaTrendCollusionEngine(pronosticos, jugadores, matches)
            ...
            print("\n" + "="*60 + "\n 🕵️‍♂️ REPORTE FORENSE DE ALIANZAS SUTILES (1X2)\n" + "="*60)
            if not reporte_fraude.empty:
                print(reporte_fraude.to_string(index=False))
            else:
                print("✅ Limpio. No se detectaron alianzas sutiles en esta jornada.")
